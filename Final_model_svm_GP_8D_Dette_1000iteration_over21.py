...
class MyGPR(GaussianProcessRegressor):
    def __init__(self, *args, max_iter=3e05, **kwargs):
...
def fun(point):
    ...
    # g(x) 
    fun_val = (value_prediction_svm(svm_classifier, point))[0] 
...
#   print('length scale {}'.format(Gaussian_regressor.kernel_.get_params()))
    # g(x) - C1*U(x)
    return fun_val**2 - C1 * log(uncertainty)
...
def mainloop(svm_classifier, num_iter, bounds, n_optimization, 
    ...
    while iter < num_iter:
        ...
        kernel = ConstantKernel(constant_value=3.0, constant_value_bounds=(0.01, 10.0)) \
                 * RBF(length_scale=sqrt(X.var()/2), length_scale_bounds='fixed') # variance will make the length scale of SVM and GP same
...
def plot_progress_plot(num_iter_list, X_initial, score_multi_list, score_random_list, score_trueran_list, title):
    ...
    plt.title(title)
    plt.xlabel('Samples')
    plt.ylabel('SVM accuracy (F1 score)')
    plt.legend()
    plt.show()
...
# Check data shape
dim = 8
num_samples = 30 # number of initial samples
...
max_itr = 1000                # number of additional samples
...
#%%
title = 'Dette8D with C1=' + str(C1) + ' and GP using SVM length scale and F1 score'
...
# Save results
np.save("F1_score_final_result_opt_1000_Dette8d_C1_100_GP_opt_over21.npy", np.array(score_multi_list))
np.save("F1_score_final_result_lhs_1000_Dette8d_C1_100_GP_opt_over21.npy", np.array(score_random_list))
np.save("F1_score_final_result_ran_1000_Dette8d_C1_100_GP_opt_over21.npy", np.array(score_trueran_list))
np.save("F1_score_extended_num_list_itr1000_Dette8d_C1_100_GP_opt_over21.npy", np.array(extended_num_itr_list))
...
plt.title('Number of samples for desired F1-score')
