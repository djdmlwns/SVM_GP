...
from svm_auxiliary_function import test_svm
...
def mainloop(svm_classifier, num_iter, bounds, n_optimization, 
    ...
    while iter < num_iter:
        ...
        kernel = ConstantKernel(constant_value=1.0, constant_value_bounds=(0.01, 1000.0)) \
                 * RBF(length_scale=sqrt(2 * X.var()), length_scale_bounds='fixed') # variance will make the length scale of SVM and GP same
        Gaussian_regressor = GaussianProcessRegressor(kernel = kernel, normalize_y = True) 
...
#        print('Length scale is {} \n'.format(Gaussian_regressor.kernel_.get_params()['k2__length_scale']))
        if ((iter+1) % 10 == 0) or (iter == 0):    
            np.random.seed()
            score = test_svm(1000, svm_classifier, initial_svm_classifier, dim, check_class)
...
#            y_random_initial = y_random.copy()
            ...
            # Test
            random_score = test_svm(1000, random_svm_classifier, random_initial_svm_classifier, dim, check_class)
...
def LHS_Random_sample_SVM(max_itr, step_size, iteration):
    ...
    for _num_iter in np.arange(0,max_itr+step_size,step_size):
        ...
        while lhs_itr < iteration:
            ...
            # Test
            random_score = test_svm(1000, trueran_svm_classifier, trueran_initial_svm_classifier, dim, check_class)
...
# Check data shape
dim = 4
num_samples = 20 # number of initial samples
...
for inner_itr in range(3):
    y = y_initial.copy()
    score_list, num_iter_list = mainloop(svm_classifier = svm_classifier,
            num_iter = max_itr, bounds = bounds, n_optimization = n_optimization, X = X, y=y)
    score_multi_list.append(score_list)

# %%
# LHS + LHS Sampling-based SVM
score_random_list = LHS_LHS_sample_SVM(max_itr = max_itr, step_size = 10, iteration = 5)

# LHS + Random sampling-based SVM
score_trueran_list = LHS_Random_sample_SVM(max_itr = max_itr, step_size= 10, iteration =5)
# %%
# Save results
...
#%%
title = 'Hertmann 4D with C1=' + str(C1) + ' and GP using SVM length scale and F1 score'
