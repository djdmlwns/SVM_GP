...
#                                print('Current score is {} \n'.format(score))
                ...
                # print('Total memory usage: {}'.format(asizeof.asizeof(self)))
                # print('SVM memory: {}'.format(asizeof.asizeof(self.SVM_classifier)))
                # print('GP memory: {}'.format(asizeof.asizeof(self.GP_regressor)))
...
#    GP_regressor = MyGPR(normalize_y = True, n_restarts_optimizer = 0, alpha = 1e-7, copy_X_train = False) 
    ...
    AS = ActiveSampling(X, y, 
                        SVM_classifier = svm.SVC(kernel='rbf', C = 10000, random_state = 42) , 
                        GP_regressor = MyGPR(normalize_y = True, n_restarts_optimizer = 0, alpha = 1e-7, copy_X_train = False) , 
                        bounds = bounds, 
                        max_itr = max_itr, case = case, C1=100, accuracy_method = accuracy_method, report_frq = report_frq,
    ...
    # print('------------------------------')
    # print('Current memory use:', memoryUse)
...
title = 'Initial_' + str(AS.X_initial.shape[0]) + '_C1_' + str(AS.C1) + '_Score_' + accuracy_method + '_Acquisition type_' + AS.acq_type + '_C1_schedule_' + str(AS.C1_schedule)# set title
