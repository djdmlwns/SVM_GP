...
condition = lambda x: Hosaki2d(x) <= -1.0
...
pwd = os.getcwd()
if os.path.exists(pwd + '/log.txt'):
    os.remove(pwd + '/log.txt')
...
for _itr in range(max_main_loop):
    ...
    AS = ActiveSampling(X, y, SVM_classifier, GP_regressor, bounds, 
                        ...
                        condition = condition, p_check = 0.0, threshold = 1, C1_schedule='None', acq_type = 'f1', log = True)
    # training start
    AS.train()
