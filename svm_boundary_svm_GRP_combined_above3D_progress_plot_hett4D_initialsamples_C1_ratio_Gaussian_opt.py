# %%
'''
In this file, the optimal gamma value of the SVM classifier for RBF kernel will be determined
by the MLE of Gaussian Process. 
'''
...
def fun(point):
    ...
    # Length scale
#    c = Gaussian_regressor.kernel_.get_params()['k1__constant_value']
    l = Gaussian_regressor.kernel_.get_params()['k2__length_scale']
    # g(x) - C1*U(x)
    return fun_val ** 2 - l**2 * np.log(uncertainty)
...
kernel = ConstantKernel(constant_value=1.0, constant_value_bounds=(0.5, 3.0)) * RBF(length_scale=1.0, length_scale_bounds=(0.1, 2.0))
...
plt.title('Hertmann 4D with ratio objective function with ratio obj and gaussian opt')
plt.xlabel('Samples')
