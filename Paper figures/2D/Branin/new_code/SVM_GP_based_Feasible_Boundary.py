...
def mainloop(svm_classifier, num_iter, bounds, n_optimization, 
    ...
    while iter < num_iter:
        ...
        kernel = ConstantKernel(constant_value=1.0, constant_value_bounds='fixed') \
                 * RBF(length_scale=sqrt(X.var() * X.shape[1]/2), length_scale_bounds='fixed') 
                #  \
...
#        print('Length scale is {} \n'.format(Gaussian_regressor.kernel_.get_params()['k2__length_scale']))
        ...
        if check_close_points(X, new_x):
            ...
            print('Iteration {0} : point x value is {1} and function value is {2:2.2E} but not added'.format(iter, new_x, new_fun))
        else:
...
n_optimization = 10            # number of optimization in the main loop for one sample
