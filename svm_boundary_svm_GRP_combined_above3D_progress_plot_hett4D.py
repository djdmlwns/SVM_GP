...
np.save("final_result_opt_200_hett4d.npy", np.array(score_list))

for _num_iter in np.arange(10,max_itr+10,10):
...
np.save("final_result_lhs_200_hett4d.npy", np.array(score_random_list))
...
np.save("final_result_ran_200_hett4d.npy", np.array(score_trueran_list))
#%%
extended_num_iter_list = np.array(num_iter_list) + X_initial.shape[0]
plt.scatter(extended_num_iter_list, score_list, color='g')
plt.plot(extended_num_iter_list, score_list, color='g', label='optimization')
