...
def mainloop(svm_classifier, Gaussian_regressor, num_iter, bounds, n_optimization, 
    ...
    while iter < num_iter:
        ...
        #################################################################################################
        # Added to introduce another way to select the sampling points with Suporting vectors
        # Pick Middle point of random two support vectors with different class
        p_sv = np.array([sv for sv in support_vectors if check_class(sv) == 1])
        n_sv = np.array([sv for sv in support_vectors if check_class(sv) == -1])

        rand_p_sv_idx = random.sample(range(0, p_sv.shape[0]), 1)
        rand_n_sv_idx = random.sample(range(0, n_sv.shape[0]), 1)
        
        p_sv_sample = p_sv[rand_p_sv_idx, :]
        n_sv_sample = n_sv[rand_n_sv_idx, :]

        de_sample = (p_sv_sample + n_sv_sample) / 2
        de_sample = de_sample.ravel()
        #################################################################################################
        ...
        # Find the optimal sampling point
        pr = random.random() 
        if pr < 0.7:
            new_x = de_sample.copy()
        else:
...
# %%
# Save results
#np.save("final_result_opt_200_hett6d_lhsinitial_c1_1_threshold02.npy", np.array(score_multi_list))
#np.save("final_result_lhs_200_hett6d_lhsinitial_c1_1_threshold02.npy", np.array(score_random_list))
#np.save("final_result_ran_200_hett6d_lhsinitial_c1_1_threshold02.npy", np.array(score_trueran_list))
#%%
...
plt.title('Hertmann 6D with C1=1 and 40% volume and 70% probability for mean support vectors')
plt.xlabel('Samples')
